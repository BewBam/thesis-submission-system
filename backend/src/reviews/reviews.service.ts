...
export class ReviewsService {
  ...
  async act(user: JwtPayload, dto: ReviewActionDto) {
    ...
    try {
      ...
      await client.query(
        ...
        [nextStatus, dto.action === "reject" ? dto.comment?.trim() || null : null, dto.submissionId, user.sub]
      );
      await client.query(
  ...
  async getAdminQueue(user: JwtPayload) {
    if (user.role !== "admin") {
      throw new ForbiddenException();
    }
  ...
  async adminAct(user: JwtPayload, dto: ReviewActionDto) {
    if (user.role !== "admin") {
      throw new ForbiddenException();
    }
    ...
    const current = await this.db.query<{ status: string }>(
      `SELECT status FROM submissions WHERE id = $1 LIMIT 1`,
      [dto.submissionId]
    );
    ...
    if (row.status !== "approving") {
      throw new BadRequestException("Submission is not in admin approval stage");
    }
